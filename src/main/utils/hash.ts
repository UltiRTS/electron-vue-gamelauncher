...
export const hashFolder = async (targetFolder: string, mode: string) => {
    if(mode == 'engine') {
        return hashElement(targetFolder, {folders: {include: folders4engine}, files: {include: ['*'], exclude: ['*.zip']}}).then((hash: any) => {
            return hash.hash;
        }).catch((e: any) => {
            console.log(e);
        })
    } else if(mode == 'mod') {
        return hashElement(targetFolder, {folders: {include: folders4mod}, files: {include: ['*'], exclude: ['*.zip']}}).then((hash: any) => {
            return hash.hash;
        }).catch((e: any) => {
            console.log(e);
        })
